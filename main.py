...
def atualizar():
    try:
        ...
        if not treev_dados:
            messagebox.showerror("Nenhum item selecionado.")
            return
        ...
        if len(tree_lista) < 9:
            messagebox.showerror("Lista de valores muito curta para preencher todos os campos de entrada.")
            return
        ...
        def update():
            ...
            mostrar()
        botao_Confirmar = tk.Button(frame_baixo,command=update, text='Atualizar', width=10, font=("ivi 9 bold"), bg=co2, fg=co1, relief='raised', overrelief='ridge')
        botao_Confirmar.place(x=110, y=510)

    except Exception as e:
        messagebox.showerror(f"Ocorreu um erro ao atualizar os campos: {e}")
...
def mostrar():
    ...
    tree.bind('<<TreeviewSelect>>', lambda event: atualizar())
